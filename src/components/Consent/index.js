...
import { userInfoSelector, loggedInSelector } from '../../state/user/user.selectors';
    ...
    <div>
      ...
      {showLogin ? (
        <div className="text-center pt-5">
          ...
          <h5>
            ...
            <span className="text-danger" onClick={() => setMode(ConsentMode.SIGNUP)}>
              {t('Consent.signUp')}
            </span>{' '}
            {t('Consent.or')} <span onClick={() => setMode(ConsentMode.LOGIN)}>{t('Consent.login')}</span>
          </h5>{' '}
        ...
        currentStep === steps.length && (
          <div>
            <div className="d-flex justify-content-center pt-3">
              <img src={'https://parkinsonmpower.org/static/images/Consent.svg'} width="55" />
      ...
      <div className="text-center">
        {!showLogin && currentStep > 0 && (
          <Button variant="outline-secondary" onClick={() => setCurrentStep(currentStep - 1)} className="step-btn mr-1">
            {t('Consent.back')}
          </Button>
        )}

        {currentStep < steps.length && (
          <Button variant="outline-success" onClick={() => setCurrentStep(currentStep + 1)} className="step-btn">
            {t('Consent.next')}
          </Button>
        )}
