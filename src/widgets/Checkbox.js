...
import { Form, Row, Card, Col, Image, OverlayTrigger, Tooltip } from 'react-bootstrap';
...
const Checkbox = ({
  ...
  const markdown = useRef(
    parseMarkdown(item.question.en, lastResponseTime, profile, props.activity, props.answers),
  ).current;
        ...
        <Col md={12}>
          <Card.Body>
            <Card.Title className="question">
              {props.watermark && <Image className="watermark" src={props.watermark} alt="watermark" rounded />}
            ...
            <div className="no-gutters">
              <Form.Group as={Row}>
                <Col md={6}>
                  {item.valueConstraints.itemList
                    .filter((obj) => !obj.isVis)
                    .map((obj, i) => (i < Math.ceil(itemCount / 2) ? renderItem(obj, i) : <></>))}
                </Col>
