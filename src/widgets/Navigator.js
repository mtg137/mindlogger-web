...
export default Navigator = (props) => {
  ...
  if (isOnePageAssessment) {
    return (
      <div className="row no-gutters d-flex flex-row justify-content-around">
        ...
        {skippable && <MyButton label={t('Consent.skip')} classes="mb-2" disabled={!isNextDisable} />}
      </div>
    );
  }
  ...
  return (
    <div className="row no-gutters d-flex flex-row justify-content-around">
      {(isBackShown && <MyButton label={t('Consent.back')} handleClick={handleBack} classes="mb-2" />) || <div />}
