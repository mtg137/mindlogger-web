...
export const getResponses = (authToken, applet) => get('response', authToken, { applet });

export const getSchedule = (authToken, timezone) => get('schedule', authToken, { timezone });
...
export const postResponse = ({ authToken, response }) =>
  postFormData(`response/${response.applet.id}/${response.activity.id}`, authToken, {
...
export const fileLink = (file, token) =>
  file ? `${apiHost()}/${file['@id']}/download?contentDisposition=inline&token=${token}` : '';
