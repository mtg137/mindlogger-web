...
export const acceptInvitation = createAsyncThunk(
  APP_CONSTANTS.ACCEPT_INVITATION,
...
export const declineInvitation = createAsyncThunk(
  APP_CONSTANTS.DECLINE_INVITATION,
  async (invitationId, { getState }) => {
    try {
      const state = getState();
      const token = authTokenSelector(state);
...
export const getInviteLinkInfo = createAsyncThunk(
  APP_CONSTANTS.GET_INVITE_LINK_INFO,
  async (inviteLinkId, { getState }) => {
    return await getInviteLinkInfoAPI({ inviteLinkId });
  },
);

export const acceptInviteLink = createAsyncThunk(
  APP_CONSTANTS.ACCEPT_INVITE_LINK,
  async (inviteLinkId, { getState }) => {
    const state = getState();
    const token = authTokenSelector(state);
