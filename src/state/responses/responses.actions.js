...
import { currentResponsesSelector, currentAppletResponsesSelector, uploadQueueSelector } from './responses.selectors';
...
export const completeResponse = createAsyncThunk(
  RESPONSE_CONSTANTS.COMPLETE_RESPONSES,
  async (isTimeout, { dispatch, getState }) => {
    let state = getState();
    ...
    if (activity.isPrize === true) {
      ...
      const updates = getTokenUpdateInfo(-selectedPrize.price, responseHistory, applet);
      ...
      if (cumActivities.length || nonHiddenCumActivities.length) {
        ...
        if (nonHiddenCumActivities.length) {
          const ids = nonHiddenCumActivities
            .map((name) => {
    ...
    setTimeout(() => {
      ...
      dispatch(removeResponseInProgress(activity.event ? activity.id + activity.event.id : activity.id));

      if (activity.allowExport) {
        sendPDFExport(authToken, applet, [activity], currentAppletResponsesSelector(state), activity.id);
      }
    }, 200);
  },
);
...
export const downloadResponses = createAsyncThunk(
  RESPONSE_CONSTANTS.DOWNLOAD_RESPONSES,
...
export const downloadResponse = createAsyncThunk(
  RESPONSE_CONSTANTS.DOWNLOAD_RESPONSE,
  async (args, { dispatch, getState }) => {
    const state = getState();
    const authToken = authTokenSelector(state);
    const userInfo = userInfoSelector(state);
...
export const startUploadQueue = createAsyncThunk(
  RESPONSE_CONSTANTS.START_UPLOAD_QUEUE,
  async (args, { dispatch, getState }) => {
    const state = getState();
