...
export const getUnscheduled = (activityList, pastActivities, scheduledActivities, finishedEvents, scheduleData) => {
  ...
  for (const activity of activityList) {
    if (!activity.events.length) {
      ...
      for (const event of activity.events) {
        ...
        if (Object.keys(finishedEvents).includes(id) || (currentEvent && !currentEvent.valid)) {
          actStatus = false;
          break;
        }
    ...
    activity.events.forEach((event) => {
      ...
      if (
        ...
        (!Object.keys(finishedEvents).includes(event.id) ||
          !moment().isSame(moment(new Date(finishedEvents[event.id])), 'day')) &&
...
export const getPastdue = (activityList, finishedEvents) => {
  ...
  activityList.forEach((activity) => {
    activity.events.forEach((event) => {
      ...
      const scheduledTime = new Date(event.scheduledTime),
        data = event.data;
      const activityTimeout = data.timeout.day * 864000000 + data.timeout.hour * 3600000 + data.timeout.minute * 60000;

      if (activity.availability) {
        if (!data.completion || !finishedEvents[event.id] || scheduledTime.getTime() > finishedEvents[event.id]) {
...
export default (activityList, inProgress, finishedEvents, scheduleData) => {
  ...
  return [
    ...addSectionHeader(addProp('status', 'in-progress', inProgressActivities), i18n.t('additional.in_progress')),
    ...addSectionHeader(
      [...addProp('status', 'pastdue', pastdue), ...addProp('status', 'unscheduled', unscheduled)],
